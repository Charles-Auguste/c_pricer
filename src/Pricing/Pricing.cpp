...
EuropeanOptionPricing::EuropeanOptionPricing(double strike, CALL_PUT call_put,
                                             double r, double T)
...
AmericanOptionPricing::AmericanOptionPricing(double strike, CALL_PUT call_put,
                                             double r, double T)
    : _strike(strike), _call_put(call_put), _r(r), _T(T) {}
...
double AmericanOptionPricing::price(Matrix asset_paths) const {
  ...
  for (size_t i = 0; i < asset_paths.size(); ++i) {
    ...
    for (size_t k = 0; k < asset_paths[i].size(); ++k) {
      cashflows[i][k] =
          max(multpilicative_factor * (asset_paths[i][k] - _strike), 0.);
    }
  }
    ...
    for (size_t j = 0; j < in_the_money.size(); ++j) {
      ...
      for (int u = k + 1; u <= N_echeances; ++u) {
        if (cashflows[idx][u] > 0 || u == N_echeances) {
          y.push_back(exp(-_r * (u - k) * _T / N_echeances) *
                      cashflows[idx][u]);
          break;
        }
      }
...
AsianOptionPricing::AsianOptionPricing(double strike, CALL_PUT call_put,
                                       double r, double T)
    : _strike(strike), _call_put(call_put), _r(r), _T(T) {}
...
double AsianOptionPricing::price(Matrix asset_paths) const {
  ...
  for (size_t k = 0; k < asset_paths.size(); k++) {
    double mean_price =
        reduce(begin(asset_paths[k]), end(asset_paths[k]), 0.0) /
        asset_paths[k].size();
