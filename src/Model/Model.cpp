...
double DupireLocalVolatilityModel::first_order_derivative_impliedvol_strike(const double& time, const double& strike) const
{
	...
	double sigma_up = _implied_volatility_surface.implied_volatility(time, strike_up);
	double sigma_down = _implied_volatility_surface.implied_volatility(time, strike_down);
	double deriv = 0.5 * (sigma_up - sigma_down) / _epsilon_maturity;
...
double DupireLocalVolatilityModel::second_order_derivative_impliedvol_strike(const double& time, const double& strike) const
{
	...
	double d_sigma_down = first_order_derivative_impliedvol_strike(time, strike_down);
...
double DupireLocalVolatilityModel::d2(const double& time, const double& strike) const{
...
double DupireLocalVolatilityModel::dupire_local_volatility(const double& time, const double& strike) const
{
	...
	double denominator = 1 + 2*d1_value*strike*d_sigma_k*sqrt(time) + d1_value*d2_value*pow(strike*d2_sigma_k,2.0)*time + pow(strike, 2.0)*time*sigma*d2_sigma_k;
