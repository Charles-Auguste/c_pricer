...
public:
  // 1) constructor
  PathSimulator(const Model *model_ptr, const double &T,
  ...
  virtual double next_step(const size_t &current_time_idx,
                           const double &current_price) const = 0;
  Vector path() const;
...
public:
  // 1) constructor
  MdPathSimulator(const MdModel *model_ptr, const double &T,
                  const size_t &nb_steps, const double &correlation);
  MdPathSimulator(const MdModel &model, const double &T, const size_t &nb_steps,
  ...
  virtual Vector next_step(size_t current_time_idx,
                           Vector current_price) const = 0;
  Vector path() const;
...
public:
  EulerPathSimulator(const Model *model_ptr, const double &T,
                     const size_t &nb_steps);
  EulerPathSimulator *clone() const override;
  double next_step(const size_t &current_time_idx,
                   const double &current_price) const override;
};

class MdEulerPathSimulator : public MdPathSimulator {
public:
  MdEulerPathSimulator(const MdModel *model_ptr, const double &T,
                       const size_t &nb_steps,
                       const double &correlation); // copy constructor
  Vector next_step(size_t current_time_idx,
                   Vector current_asset_value) const override;
  MdEulerPathSimulator *clone() const override;
};

class BroadieKayaPathSimulator : public MdPathSimulator {
public:
  BroadieKayaPathSimulator(const HestonModel *model_ptr, const double &T,
                           const size_t &nb_steps,
                           const double &correlation); // copy constructor
  Vector next_step(size_t current_time_idx,
                   Vector current_asset_value) const override;
...
private:
  ...
  double integral_sampling(size_t current_time_index, double var_current_value)
      const; // estimation of the integral of V(s)ds between t and t+dt given
             // V_t and V_{t+dt} (by var_next_step)

  // Next Step to go from S_t to S_{t + Delta t} :
  double var_next_step(size_t current_time_idx, double current_value) const;
  double asset_next_step(size_t current_time_idx,
                         const Vector current_asset_log_value) const;
};