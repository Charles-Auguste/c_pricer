...
// PathSimulator class
PathSimulator::PathSimulator(const Model* model_ptr, const double& T, const size_t& nb_steps)
	: _model_ptr(model_ptr->clone()), _T(T), _nb_steps(nb_steps)
{
	build_time();
}

PathSimulator::PathSimulator(const Model& model, const double& T, const size_t& nb_steps)
...
PathSimulator::PathSimulator(const PathSimulator& path_simulator)
	: _model_ptr(path_simulator._model_ptr->clone()), _T(path_simulator._T), _nb_steps(path_simulator._nb_steps)
{
	_time_points = path_simulator._time_points;
}

PathSimulator::PathSimulator(PathSimulator&& path_simulator) noexcept
	: _model_ptr(std::move(path_simulator._model_ptr)), _T(path_simulator._T), _nb_steps(path_simulator._nb_steps)
{
...
EulerPathSimulator::EulerPathSimulator(const Model* model_ptr, const double& T, const size_t& nb_steps)
        : PathSimulator(model_ptr, T, nb_steps)
    {}

double EulerPathSimulator::next_step(const size_t& current_time_idx, const double& current_price) const
{
...
MdPathSimulator::MdPathSimulator(const MdModel* model_ptr, const double& T, const size_t& nb_steps, const double& correlation)
	: _model_ptr(model_ptr->clone()), _T(T), _nb_steps(nb_steps), _correlation(correlation)
{
	assert(model_ptr->init_value().size() == 2);
	build_time();
}

MdPathSimulator::MdPathSimulator(const MdModel& model, const double& T, const size_t& nb_steps, const double& correlation)
	: _model_ptr(model.clone()), _T(T), _nb_steps(nb_steps), _correlation(correlation)
{
...
MdPathSimulator::MdPathSimulator(const MdPathSimulator& path_simulator)
	: _model_ptr(path_simulator._model_ptr->clone()), _T(path_simulator._T), _nb_steps(path_simulator._nb_steps), _correlation(path_simulator._correlation)
{
	_time_points = path_simulator._time_points;
}

MdPathSimulator::MdPathSimulator(MdPathSimulator&& path_simulator) noexcept
	: _model_ptr(std::move(path_simulator._model_ptr)), _T(path_simulator._T), _nb_steps(path_simulator._nb_steps), _correlation(path_simulator._correlation)
{
	_time_points = std::move(path_simulator._time_points);
}

MdPathSimulator& MdPathSimulator::operator=(const MdPathSimulator& path_simulator)
{
	if (this != &path_simulator)
	{
		delete _model_ptr;
...
MdEulerPathSimulator::MdEulerPathSimulator(const MdModel* model_ptr, const double& T, const size_t& nb_steps, const double& correlation):
	MdPathSimulator(model_ptr, T, nb_steps, correlation)
{
}

Vector MdEulerPathSimulator::next_step(size_t current_time_idx, Vector current_asset_value) const
{
	std::random_device rd;
	std::mt19937 gen(rd());
	...
	next_log_price = current_asset_value[0] 
					 ...
					 + _model_ptr->diffusion_vector_term(_time_points[current_time_idx], current_asset_value)[0] * sqrt(delta_t) * g1;
	next_var       = current_asset_value[1] 
					 + _model_ptr->drift_vector_term(_time_points[current_time_idx], current_asset_value)[1] * delta_t
					 + _model_ptr->diffusion_vector_term(_time_points[current_time_idx], current_asset_value)[1] * sqrt(delta_t) * g2;
...
Vector BroadieKayaPathSimulator::next_step(size_t current_time_idx, Vector current_asset_value) const
{
	std::random_device rd;
	std::mt19937 gen(rd());
...
double BroadieKayaPathSimulator::integral_sampling(size_t current_time_index, double var_current_value) const 
{
	double var_next_value = var_next_step(current_time_index, var_current_value);
	return 0.0;
};

double BroadieKayaPathSimulator::var_next_step(size_t current_time_idx, double current_value) const
{
	double delta_t = _time_points[current_time_idx + 1] - _time_points[current_time_idx];
	...
	double next_var = pre_factor * chi_prime(degree, current_value * std::exp(-_model_ptr->kappa() * delta_t) / pre_factor); // where f is loi du chi 2 � d degr�s de libert� d�riv�
	return next_var;
};

double BroadieKayaPathSimulator::asset_next_step(size_t current_time_idx, const Vector current_asset_log_value) const
{
	std::random_device rd;
	std::mt19937 gen(rd());
	std::normal_distribution<double> distribution(0, 1.); // N(0,1)
