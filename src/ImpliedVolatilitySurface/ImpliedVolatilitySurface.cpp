...
ImpliedVolatilitySurface::ImpliedVolatilitySurface(
    const Vector &maturities, const Vector &strikes,
    const Matrix &market_implied_volatilities, const double &risk_free_rate)
...
bool ImpliedVolatilitySurface::check_ordered_vectors() const {
  return std::is_sorted(_maturities.begin(), _maturities.end()) &&
         std::is_sorted(_strikes.begin(), _strikes.end()) &&
         _maturities[0] > 0 && _strikes[0] >= 0;
}

bool ImpliedVolatilitySurface::check_dimensions() const {
  return _market_implied_volatilities.size() == _maturities.size() &&
         _market_implied_volatilities[0].size() == _strikes.size();
}
...
double
ImpliedVolatilitySurface::implied_volatility(const double &maturity,
                                             const double &strike) const {
  // I don't want to write everything in this method
  // It re-uses the private smile_implied_Volatility by using the linear
  // interpolation in variance or extrapolation

  if (maturity < _maturities[0]) {
    double strike_0 =
        strike * std::exp(_risk_free_rate * (_maturities[0] - maturity));
    return smile_implied_volatility(0, strike_0);
  } else if (maturity > _maturities[_maturities.size() - 1]) {
    double strike_M =
        strike * std::exp(_risk_free_rate *
                          (_maturities[_maturities.size() - 1] - maturity));
  ...
  // Evaluate K^(i) and K^(i+1) - No need for S_0, just rate is needed, hence
  // passed as a private data of the object.
  double left_strike = strike * std::exp(_risk_free_rate *
                                         (_maturities[lower_index] - maturity));
  double right_strike =
      strike *
      std::exp(_risk_free_rate * (_maturities[upper_index] - maturity));

  // Compute v(T_i, k_F) and v(T_{i+1}, k_F) : use of cubic spline
  // interpolation/extrapolation
  double left_implied_vol = smile_implied_volatility(lower_index, left_strike);
  double left_total_variance =
      left_implied_vol * left_implied_vol * _maturities[lower_index];

  double right_implied_vol =
      smile_implied_volatility(upper_index, right_strike);
  double right_total_variance =
      right_implied_vol * right_implied_vol * _maturities[upper_index];

  // Linear interpolation in variance along maturities
  double implied_total_variance =
      left_total_variance +
      ((right_total_variance - left_total_variance) /
       (_maturities[upper_index] - _maturities[lower_index])) *
          (maturity - _maturities[lower_index]);
...
void ImpliedVolatilitySurface::evaluate_cubic_spline_coefficients() {
  for (size_t mat_idx = 0; mat_idx < _maturities.size(); ++mat_idx) {
    ...
    for (size_t j = 0; j < strike_size() - 2; ++j) {
      ...
      rhs.push_back(3. * ((Y[j + 2] - Y[j + 1]) / _delta_strikes[j + 1] -
    ...
    alpha.push_back(-beta[_strikes.size() - 2] /
                    (3. * _delta_strikes[_strikes.size() - 2]));
    _alpha_coefficients.push_back(alpha);

    Vector gamma; // maturity T[mat_idx]
    for (size_t j = 0; j < _strikes.size() - 1; ++j)
      gamma.push_back((Y[j + 1] - Y[j]) / _delta_strikes[j] -
...
double
ImpliedVolatilitySurface::smile_implied_volatility(const size_t &maturity_index,
                                                   const double &strike) const {
  double implied_vol;

  // Locate the strike in the right strike interval from _strikes : : K in [K_j,
  // K_{j+1}] or K < K_1 or K > K_N
  ...
  if (strike <= _strikes[0]) {
    ...
    implied_vol = _market_implied_volatilities[maturity_index][0] +
                  deriv_left * (strike - _strikes[0]);
  } else {
    // right extrapolation case
    if (strike >= _strikes[strike_size() - 1]) {
      // sigma(T_i, K) = sigma(T_i, K_N) + (3 alpha_{N-1} Delta^2 x_{N-1} + 2
      // beta_{N-1} Delta x_{N-1} + gamma_{N-1}) * (K - K_N)
      double deriv_right =
          3. * _alpha_coefficients[maturity_index][strike_size() - 2] *
              _delta_strikes[strike_size() - 2] *
              _delta_strikes[strike_size() - 2] +
          2. * _beta_coefficients[maturity_index][strike_size() - 2] *
              _delta_strikes[strike_size() - 2] +
          _gamma_coefficients[maturity_index][strike_size() - 2];
      implied_vol =
          _market_implied_volatilities[maturity_index][strike_size() - 1] +
          deriv_right * (strike - _strikes[strike_size() - 1]);
    } else
    // interpolation case
    {
      ...
      implied_vol =
          alpha * dx * dx * dx + beta * dx * dx + gamma * dx +
          _market_implied_volatilities[maturity_index][left_strike_index];
    }
  }
