...
ThomasSolver::ThomasSolver(const Vector &lower_diag, const Vector &central_diag,
                           const Vector &upper_diag, const Vector &rhs)
    : _lower_diagonal(lower_diag), _central_diagonal(central_diag),
      _upper_diagonal(upper_diag), _right_hand_side(rhs) {}
...
Vector ThomasSolver::solve() const {
  ...
  for (size_t j = 1; j < rhs_size - 1; ++j)
    upper_diag_prime.push_back(
        _upper_diagonal[j] /
        (_central_diagonal[j] -
         _lower_diagonal[j - 1] * upper_diag_prime[j - 1]));

  Vector rhs_prime;
  rhs_prime.push_back(_right_hand_side[0] / _central_diagonal[0]);
  for (size_t j = 1; j < rhs_size; ++j)
    rhs_prime.push_back(
        (_right_hand_side[j] - _lower_diagonal[j - 1] * rhs_prime[j - 1]) /
        (_central_diagonal[j] -
         _lower_diagonal[j - 1] * upper_diag_prime[j - 1]));
  ...
  // for (size_t j = rhs_size - 2; j >= 0; --j) Does not work -> infinite loop
  // -> -1 = + infty [max number of size_t]
