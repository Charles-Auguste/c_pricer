...
// ExplicitModel
ExplicitModel::ExplicitModel() : _S(100.), _r(0.05) {}

ExplicitModel::ExplicitModel(const double &S, const double &r) : _S(S), _r(r) {}

ExplicitModel::ExplicitModel(const ExplicitModel &model)
    : _S(model._S), _r(model._r) {}
...
void ExplicitModel::SetS(const double &S) { _S = S; }

void ExplicitModel::SetR(const double &r) { _r = r; }

double ExplicitModel::GetS() const { return _S; }

double ExplicitModel::GetR() const { return _r; }

// ExplicitBlackScholesModel
ExplicitBlackScholesModel::ExplicitBlackScholesModel()
    : ExplicitModel(), _sigma(0.2) {}

ExplicitBlackScholesModel::ExplicitBlackScholesModel(const double &S,
                                                     const double &r,
                                                     const double &sigma)
    : ExplicitModel(S, r), _sigma(sigma) {}

ExplicitBlackScholesModel::ExplicitBlackScholesModel(
    const ExplicitBlackScholesModel &model)
    : ExplicitModel(model), _sigma(model._sigma) {}
...
double ExplicitBlackScholesModel::GetSigma() const { return _sigma; }
...
double ExplicitBlackScholesModel::CallPrice(const double &K,
                                            const double &T) const {
  double d1 =
      (log(_S / K) + (_r + 0.5 * _sigma * _sigma) * T) / (_sigma * pow(T, 0.5));
  double d2 = d1 - _sigma * pow(T, 0.5);
  return _S * normalCDF(d1) - K * exp(-_r * T) * normalCDF(d2);
}

double ExplicitBlackScholesModel::Vega(const double &K, const double &T,
                                       const double &sigma_mkt) const {
  double d1 = (log(_S / K) + (_r + 0.5 * sigma_mkt * sigma_mkt) * T) /
              (sigma_mkt * pow(T, 0.5));
...
ExplicitHestonModel::ExplicitHestonModel()
    : ExplicitModel(), _kappa(1), _theta(0.2), _sigma(0.2), _rho(0), _v0(0.2) {}

ExplicitHestonModel::ExplicitHestonModel(const double &kappa,
                                         const double &theta,
                                         const double &sigma, const double &rho,
                                         const double &v0, const double &S,
                                         const double &r)
    : ExplicitModel(S, r), _kappa(kappa), _theta(theta), _sigma(sigma),
      _rho(rho), _v0(v0){};

ExplicitHestonModel::ExplicitHestonModel(const ExplicitHestonModel &other)
    : ExplicitModel(other), _kappa(other._kappa), _theta(other._theta),
      _sigma(other._sigma), _rho(other._rho), _v0(other._v0){};
...
double ExplicitHestonModel::GetKappa() const { return _kappa; }

double ExplicitHestonModel::GetTheta() const { return _theta; }

double ExplicitHestonModel::GetSigma() const { return _sigma; }

double ExplicitHestonModel::GetRho() const { return _rho; }

double ExplicitHestonModel::GetV0() const { return _v0; }

void ExplicitHestonModel::SetKappa(const double &kappa) { _kappa = kappa; }

void ExplicitHestonModel::SetTheta(const double &theta) { _theta = theta; }

void ExplicitHestonModel::SetSigma(const double &sigma) { _sigma = sigma; }

void ExplicitHestonModel::SetRho(const double &rho) { _rho = rho; }
...
Complex ExplicitHestonModel::C1(const double &T, const double &omega) const {
  ...
  Complex C =
      _r * i * omega * 0.5 * T * T +
      ((_kappa * _theta) / (_sigma * _sigma)) *
          ((a - b) * T - 2.0 * log((1.0 - g * exp(-b * T)) / (1.0 - g)));
...
Complex ExplicitHestonModel::D1(const double &T, const double &omega) const {
  ...
  Complex D = ((a - b) / (_sigma * _sigma)) *
              ((1.0 - exp(-b * T)) / (1.0 - g * exp(-b * T)));
...
Vector ExplicitHestonModel::gauss_hermite_points() const {
  ...
  return {-10.15910925, -9.52090368, -8.992398,   -8.52056928, -8.08518865,
          -7.67583994,  -7.28627659, -6.91238153, -6.55125917, -6.20077356,
          -5.8592902,   -5.52552109, -5.19842653, -4.87715008, -4.56097376,
          -4.24928644,  -3.94156073, -3.63733588, -3.33620465, -3.03780334,
          -2.74180375,  -2.4479069,  -2.15583787, -1.86534153, -1.57617901,
          -1.28812467,  -1.0009635,  -0.71448878, -0.42850006, -0.14280124,
          0.14280124,   0.42850006,  0.71448878,  1.0009635,   1.28812467,
          1.57617901,   1.86534153,  2.15583787,  2.4479069,   2.74180375,
          3.03780334,   3.33620465,  3.63733588,  3.94156073,  4.24928644,
          4.56097376,   4.87715008,  5.19842653,  5.52552109,  5.8592902,
          6.20077356,   6.55125917,  6.91238153,  7.28627659,  7.67583994,
...
Vector ExplicitHestonModel::P_1_2(const double &K, const double &T) const {
  ...
  for (int k = 0; k < N; k++) {
    ...
    omega = points[k];
    integrale1 +=
        weights[k] * ((phi1(T, omega) *
                       exp((omega * omega) - i * omega * logK) / (i * omega)))
                         .real();
    integrale2 +=
        weights[k] * ((phi2(T, omega) *
                       exp((omega * omega) - i * omega * logK) / (i * omega)))
                         .real();
  }
...
OptimisationImpliedVolatility::OptimisationImpliedVolatility()
    : _epsilon_iv(0.005), _model_ptr(new ExplicitHestonModel),
      _bs_model_ptr(new ExplicitBlackScholesModel) {}

OptimisationImpliedVolatility::OptimisationImpliedVolatility(
    const double &epsilon, const ExplicitHestonModel &model,
    const ExplicitBlackScholesModel &bs_model)
    : _epsilon_iv(epsilon), _model_ptr(model.clone()),
      _bs_model_ptr(bs_model.clone())

          {};

OptimisationImpliedVolatility::OptimisationImpliedVolatility(
    const OptimisationImpliedVolatility &optim)
    : _epsilon_iv(optim._epsilon_iv), _model_ptr(optim._model_ptr),
...
ExplicitBlackScholesModel *
OptimisationImpliedVolatility::GetBSModelPtr() const {
  return _bs_model_ptr;
}

double OptimisationImpliedVolatility::GetEpsilon() const { return _epsilon_iv; }
...
double OptimisationImpliedVolatility::loss_function_square(
